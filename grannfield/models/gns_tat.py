...
from grannfield.utils.utils import radius_graph_pbc, get_pbc_distances, compute_neighbors
...
class GNS_TAT(nn.Module):
    ...
    def _forward(self, data):
        ...
        #Pooling
        x = [torch.mean(x[idx_map], dim=0, keepdim=True) for idx_map in data['atom_idx']]
        x = torch.cat(x, dim=0)
