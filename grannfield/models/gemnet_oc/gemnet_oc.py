"""
Copyright (c) 2022 Salatan Duangdangchote, Clean Energy Lab at the University of Toronto Scarborough.

Licensed under the MIT License (MIT), see
salatandua/grannip: https://github.com/salatandua/grannip
...
def generate_graph(
    ...
    max_neighbors = max_neighbors or max_neighbors
    ...
    distances_vec = out["distances_vec"]
...
class GemNet_OC(ScaledModule, nn.Module):
    """
    Arguments
    ---------
    ...
    def generate_graph_dict(self, data, cutoff, max_neighbors):
        ...
        edge_vector = -distances_vec / edge_dist[:, None]
    ...
    def get_graphs_and_indices(self, data):
        ...
        main_graph, id_swap = self.symmetrize_edges(main_graph, data)
    ...
    def forward(self, data):
        ...
        # Embedding block
        h = self.atom_emb(data['atomic_numbers'])
        ...
        if self.extensive:
            E_t = [torch.sum(E_t[idx_map], dim=0, keepdim=True) for idx_map in data['atom_idx']]
            E_t = torch.cat(E_t, dim=0)
        else:
            E_t = [torch.mean(E_t[idx_map], dim=0, keepdim=True) for idx_map in data['atom_idx']]
            E_t = torch.cat(E_t, dim=0)
