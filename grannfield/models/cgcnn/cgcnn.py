...
class CGCNN(nn.Module):
    ...
    def _forward(self, data):
        ...
        if self.use_pbc:
            neighbors_index, cell_offsets, neighbors, distances = radius_graph_pbc(
            ...
            data['neighbors_index'] = radius_graph(
                data['positions'], r=self.cutoff
            )
            row, col = data['neighbors_index']
    ...
    def _convolve(self, data):
        ...
        mol_feats = [torch.mean(node_feats[idx_map], dim=0, keepdim=True) for idx_map in data['atom_idx']]
