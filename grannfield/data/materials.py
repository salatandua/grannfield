...
def _convert_atoms(
    ...
    inputs['atomic_numbers'] = np.array(atoms.numbers, dtype=np.int)
    positions = atoms.positions.astype(np.float32)
    inputs['n_atoms'] = np.array([positions.shape[0]], dtype=np.int)
    ...
    fixed_idx = np.zeros(positions.shape[0], dtype=np.int)
...
class AtomsConverter:
    ...
    def __call__(self, atoms):
        ...
        inputs['atom_idx'] = [torch.LongTensor(np.arange(n_i))]
        ...
        for key, value in inputs.items():
            ...
            if key == 'atom_idx':
                inputs[key] = [idx.to(self.device) for idx in value]
